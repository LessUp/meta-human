...
vi.mock('lucide-react', () => {
  const handler = {
    get(_target: any, prop: string) {
      ...
      // 返回一个简单的 SVG 组件
      return (props: any) => React.createElement('svg', { 'data-testid': `icon-${prop}`, ...props });
    }
  };
...
describe('ExpressionControlPanel', () => {
  ...
  it('点击微表情按钮触发回调', () => {
    ...
    expect(defaultProps.onExpressionChange).toHaveBeenCalledWith('eyebrow_raise', expect.any(Number));
  });
  ...
  it('渲染重置表情按钮', () => {
    ...
    expect(defaultProps.onExpressionChange).toHaveBeenCalledWith('neutral', 0.5);
  });

  it('当前表情按钮高亮显示', () => {
    render(<ExpressionControlPanel {...defaultProps} currentExpression="smile" />);
    // 显示 smile 标签
