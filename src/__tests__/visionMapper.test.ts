...
describe('mapFaceToEmotion', () => {
  ...
  it('detects surprised from wide eyes and open mouth', () => {
    ...
    const result = mapFaceToEmotion(
      buildResults({
        159: { x: 0.37, y: 0.3, z: 0 }, // leftEyeUpper
        145: { x: 0.37, y: 0.5, z: 0 }, // leftEyeLower
        133: { x: 0.35, y: 0.4, z: 0 }, // leftEyeInner
        33: { x: 0.39, y: 0.4, z: 0 }, // leftEyeOuter
        386: { x: 0.63, y: 0.3, z: 0 }, // rightEyeUpper
        374: { x: 0.63, y: 0.5, z: 0 }, // rightEyeLower
        362: { x: 0.61, y: 0.4, z: 0 }, // rightEyeInner
        263: { x: 0.65, y: 0.4, z: 0 }, // rightEyeOuter
        61: { x: 0.35, y: 0.65, z: 0 }, // leftMouth
        291: { x: 0.65, y: 0.65, z: 0 }, // rightMouth
        13: { x: 0.5, y: 0.6, z: 0 }, // upperLip
        14: { x: 0.5, y: 0.8, z: 0 }, // lowerLip (wide open)
      }),
    );
  ...
  it('detects angry from brow lowered and mouth corners down', () => {
    ...
    const result = mapFaceToEmotion(
      buildResults({
        ...
        133: { x: 0.34, y: 0.4, z: 0 }, // leftEyeInner
        33: { x: 0.42, y: 0.4, z: 0 }, // leftEyeOuter (wide)
        386: { x: 0.63, y: 0.39, z: 0 }, // rightEyeUpper
        374: { x: 0.63, y: 0.41, z: 0 }, // rightEyeLower
        362: { x: 0.58, y: 0.4, z: 0 }, // rightEyeInner
        263: { x: 0.66, y: 0.4, z: 0 }, // rightEyeOuter (wide)
  ...
  it('detects sad from mouth corners down and brows up', () => {
    ...
    const result = mapFaceToEmotion(
      buildResults({
        ...
        362: { x: 0.58, y: 0.4, z: 0 }, // rightEyeInner
        263: { x: 0.66, y: 0.4, z: 0 }, // rightEyeOuter (wide)
        107: { x: 0.37, y: 0.2, z: 0 }, // leftEyebrowInner (raised)
        336: { x: 0.63, y: 0.2, z: 0 }, // rightEyebrowInner (raised)
        61: { x: 0.35, y: 0.8, z: 0 }, // leftMouth (pulled down a lot)
        291: { x: 0.65, y: 0.8, z: 0 }, // rightMouth (pulled down a lot)
        13: { x: 0.5, y: 0.7, z: 0 }, // upperLip
...
describe('analyzeFaceFeatures', () => {
  ...
  it('returns null for insufficient landmarks', () => {
    const short = {
      multiFaceLandmarks: [Array.from({ length: 100 }, () => ({ x: 0.5, y: 0.5, z: 0 }))],
    };
    expect(analyzeFaceFeatures(short)).toBeNull();
  });

  it('returns feature values for valid input', () => {
    const features = analyzeFaceFeatures(
      buildResults({
        159: { x: 0.37, y: 0.3, z: 0 },
        145: { x: 0.37, y: 0.4, z: 0 },
        133: { x: 0.35, y: 0.35, z: 0 },
        33: { x: 0.39, y: 0.35, z: 0 },
        386: { x: 0.63, y: 0.3, z: 0 },
        374: { x: 0.63, y: 0.4, z: 0 },
