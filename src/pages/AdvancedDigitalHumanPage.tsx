  ...
  return (
    <div className="relative w-screen h-screen bg-slate-100 dark:bg-black overflow-hidden font-sans text-slate-800 dark:text-white selection:bg-blue-500/30">
      ...
      <div className="absolute top-0 left-0 w-full p-6 z-20 flex justify-between items-start pointer-events-none">
        <div className="pointer-events-auto">
          ...
          <div className="mt-2 flex space-x-4 text-xs text-slate-500 dark:text-gray-400 font-mono">
            ...
            <span>
              会话:{" "}
              <span className="text-purple-400">{chatHistory.length}条</span>
            </span>
            <span>
              FPS:{" "}
              <span
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 w-full max-w-2xl px-4 z-20">
          ...
          {chatHistory.length === 0 ? (
            <div className="text-center py-12 space-y-3">
              ...
              <p className="text-slate-300 dark:text-white/15 text-xs">
                按{" "}
                <kbd className="px-1.5 py-0.5 bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded text-[10px]">
                  V
                </kbd>{" "}
                快速开始录音
              </p>
            ...
            chatHistory.map((msg) => (
              ...
              >
                <div
                  className={`max-w-[80%] px-5 py-3 rounded-2xl text-sm backdrop-blur-md border shadow-sm ${
                    msg.role === "user"
        ...
        {/* Input Bar */}
        <div
          className={`bg-white/80 dark:bg-black/60 backdrop-blur-2xl border rounded-2xl p-2 pl-4 flex items-center gap-3 shadow-lg shadow-slate-200/50 dark:shadow-blue-900/20 ring-1 ring-slate-100 dark:ring-white/5 transition-colors ${
            isLoading
              ? "border-blue-500/50"
