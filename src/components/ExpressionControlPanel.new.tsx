...
export default function ExpressionControlPanel({ currentExpression, onExpressionChange }: ExpressionControlPanelProps) {
  ...
  const expressions: ExpressionControl[] = [
    { name: 'neutral', label: '中性', icon: <Meh size={20} />, color: 'text-gray-400', intensity: 0.5 },
    ...
    { name: 'angry', label: '愤怒', icon: <Angry size={20} />, color: 'text-red-400', intensity: 0.9 },
  ...
  return (
    <div className="space-y-8">
          ...
          <button
            ...
            className={`flex items-center space-x-3 p-3 rounded-xl border transition-all duration-200 group text-left ${
              currentExpression === expression.name
                ...
                : 'border-slate-200 dark:border-white/5 bg-white/50 dark:bg-white/5 hover:bg-slate-50 dark:hover:bg-white/10 hover:border-slate-300 dark:hover:border-white/10'
            }`}
          >
            <div className={`p-2 rounded-lg bg-slate-100 dark:bg-black/20 ${expression.color}`}>
              {expression.icon}
            </div>
            <div>
      ...
      <div className="space-y-3">
        <h4 className="text-xs font-semibold text-slate-400 dark:text-white/40 uppercase tracking-wider">微表情</h4>
        <div className="grid grid-cols-2 gap-2">
          {[
            { label: '抬眉', key: 'eyebrow_raise' },
            { label: '快速眨眼', key: 'eye_blink' },
            { label: '张嘴', key: 'mouth_open' },
            { label: '点头', key: 'head_nod' },
          ].map((action) => (
            <button
              key={action.key}
              onClick={() => onExpressionChange(action.key, intensity)}
              className="px-3 py-2 bg-white/50 dark:bg-white/5 hover:bg-slate-50 dark:hover:bg-white/10 text-slate-600 dark:text-white/70 hover:text-slate-800 dark:hover:text-white border border-slate-200 dark:border-white/5 rounded-lg text-xs transition-colors"
            >
              {action.label}
            </button>
          ))}
